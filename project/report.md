# AI REST Service using Open API on Google and AWS
...
Random Forest consists of a large number of decision trees, each of which makes a class prediction. The prediction that the majority of the trees make becomes the model's prediction. To create a Random Forest application, the first step is to create a simple service that demonstrates the random forest function. The random forest python file consists of three functions, *upload_file*, *fit* and *predict*. The csv file that we want to use is uploaded in the *upload* function. In the *fit* function, for every csv file that is uploaded, the last column is designated as our target column and a model is fitted using the parameters provided. The *predict* function uses the model created using *fit* to classify the data. The server file is used to create an application instance as well as a URL route for the application. The YAML file consists of the same endpoints that are mentioned as fuvntions in the python file. The URL to access the application is also specified. All of these required files are bundled in an app and a Dockerfile is added as well. This same file is used to build an image later on for deploying onto the cloud services. The dockerfile specifies a set of commands that tell the system what to do, such as installing the requirements and running the server file. This server file in turn points to the yaml file that specifies and configures the end points for our application. 
