...
default_model_params = {'n_neighbours': 5}

INPUT_DIR = '/tmp/rest-knn-test/input'
os.makedirs(INPUT_DIR, exist_ok=True)

OUTPUT_DIR = '/tmp/rest-knn-test/output'
...
def knn_fit(body):
    ...
    knn = KNeighborsClassifier(**params).fit(X)

    models.update({job_id: knn})  # add the model in to the dict

    labels = OUTPUT_DIR + "/" + str(job_id) + ".labels"
    np.savetxt(labels, knn.labels_, delimiter=",")

    return send_file(labels)


def knn_predict(body, file=None):
